...
import com.example.ttapi.repository.IFacturaRespository;
import com.example.ttapi.repository.IPersonaRepository;
...
public class VentasServiceImpl implements IVentasService {
    ...
    public ResponseEntity<Object> findFacturasByPersona(Long personaId) {
        if (personaId != null && personaId > 0) {
            try {
                logger.info("Buscando facturas para la persona del id = " + personaId);
                List<Factura> facturas = facturaRespository.getAllByPersonaId(personaId);
                return new ResponseEntity<>(facturas, HttpStatus.OK);
            } catch (Exception e) {
                logger.error("Ocurrió un error al buscar las facturas para la persona del id = " + personaId
                        + "\nRazón: " + e.getMessage());
    ...
    public ResponseEntity<Object> storeFactura(Factura factura) {
        if (factura.getPersonaId() != null) {
            ...
            if (persona != null && factura.getMonto().compareTo(BigDecimal.ZERO) > 0 && factura.getFecha() != null) {
                ...
                logger.warn("No se encontó a la persona = " + factura.getPersonaId());
            } else {
